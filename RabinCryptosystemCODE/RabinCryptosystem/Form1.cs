...
namespace RabinCryptosystem
{
    ...
    {
        private const uint bytesToWrite = 30;
        ...
        private string GetBytesRepresentation(byte[] bytes,uint ind,uint count)
        {
            StringBuilder sb = new StringBuilder();
            for (uint i = ind; i < ind + count - 1; i++)
            {
                sb.Append(bytes[i]);
                sb.Append(' ');
            }
            sb.Append(bytes[count - 1]);
            return sb.ToString();
        }
        ...
        private void ShowResult(TextBox text, byte[] bytes)
        {
            string res = "";
            if (bytes.Length > text.MaxLength)
            {
                res += "First: ";
                res += GetBytesRepresentation(bytes, 0, RabinCryptosystem.bytesToWrite);
                res += "\r\n";
                res += "Last: ";
                res += GetBytesRepresentation(bytes, (uint)(bytes.Length - RabinCryptosystem.bytesToWrite), RabinCryptosystem.bytesToWrite);
            }
            else
            {
                res = GetBytesRepresentation(bytes, 0, (uint)bytes.Length);
            }
            text.Text = res;
        }
